...
forbidden_paths=$(grep -Ev \
  '^(moon\.work|scripts/run-minimal-no-shadow-lambda-projection\.sh|scripts/run-minimal-no-shadow-lambda-cutoff-benchmark\.sh|scripts/run-minimal-no-shadow-lambda-cutoff-mixed-benchmark\.sh|examples/spikes/minimal_no_shadow_lambda_projection(/.*)?|modules/canopy/lang/lambda/companion/minimal_no_shadow_p1_evidence\.mbt|modules/canopy/lang/lambda/companion/pkg\.generated\.mbti)$' \
  <<<"$changed_paths" || true)
