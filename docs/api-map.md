...
| API | Location | Notes |
|-----|----------|-------|
| `SourceSnapshot` | `modules/analysis/` | Doc identity: doc_id + version + 32-bit text_hash + utf16_len. Constructed from source text — do not build field-by-field. |
| `SourceSnapshot::SourceSnapshot(doc_id~, version~, source~)` | `modules/analysis/` | Computes text_hash and utf16_len from source string. |
| `SourceSnapshot::matches(other)` | `modules/analysis/` | Full identity check (all four fields). Stale-result gate. |
| `PatternMatchFact` | `modules/analysis/` | Snapshot-bound match: UTF-16 from/to, pattern_id, captures map. |
| `PatternMatchFact::is_current(snapshot)` | `modules/analysis/` | Returns false when fact's snapshot differs from current by doc_id, version, hash, or length. |
| `byte_offset_to_utf16(source, byte_offset)` | `modules/analysis/` | Adapter-boundary conversion: ast-grep UTF-8 byte offset → UTF-16 code units. Call only at the adapter boundary. |
