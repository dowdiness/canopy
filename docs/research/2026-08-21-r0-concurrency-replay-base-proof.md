...
4. **Fetch exactly these payloads, excluding bytes already supplied.** After the replay set is fixed, derive one stable topological replay order with Kahn's algorithm over declared-parent edges whose endpoints are both in the above-base replay set. An event is ready when every in-set declared parent has been emitted; parents outside the set must already be proven at/below the selected base. The ready set is a min-priority queue ordered by canonical raw identity (unsigned lexical UTF-8 agent bytes, then numeric sequence). Insert newly ready nodes with the same comparator and replay exactly the popped sequence; implicit predecessors and semantic references are verified readiness facts, never added as topological edges. A cycle, missing in-set parent, or incomplete output is authenticated corruption/fallback. Fetch payloads for exactly that ordered identity set in stable batches. Cold payload requests exclude payload bytes already supplied by the incoming batch or hot input; only bytes not already available are counted as cold provider payload reads. The fixture catalog stores the expected ordered replay IDs for every registered concurrency case, and `oracle-differential.jsonl` records expected/actual order; destination LV or admission/storage order is never a tie-break.
