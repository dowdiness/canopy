...
4. A separate fresh Node/JS probe consumer receives the published exact candidate bytes plus the pinned read-only fixture catalog, and re-hashes the immutable snapshot framing, document text byte/scalar lengths, and sorted heads through executable-local codec/validation surfaces. It does not independently recompute `graph_root_v2` from cold event payloads on the fast path. The Markdown/Loomark black-box harness receives only complete v1 archive bytes, never candidate/frontier bytes, and emits full-history oracle observations joined later by `run_id` and `case_id`.
