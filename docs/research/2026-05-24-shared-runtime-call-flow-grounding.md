...
**Post-language-SPI note (2026-08-07):** references below to `LanguageSpec` and `SyncEditor::new_generic` describe the grounded 2026-05-24 call flow. The current chain is `Language::build` → `SyncEditor::new_with_builder`; it preserves the same `parent_runtime?` threading while owning parser, memos, capabilities, and the opaque identity-hint consumer coherently.
