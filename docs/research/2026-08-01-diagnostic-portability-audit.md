...
`modules/canopy/probe/diagnostic_portability/top.mbt:4-76`: its source identity is
...
| Label survival | Semantic production and its test preserve `Primary` and `Secondary` (`lang/lambda/semantic/semantic_projection.mbt:103-141`; `semantic_projection_test.mbt:197-231`), and the generic renderer prints both (`loom/loom/core/diagnostic_renderer_wbtest.mbt:92-130`). The audited Lambda parser diagnostic has only `Primary` (`loom/examples/lambda/cst_parser.mbt:157-165`), and the probe asserts only that output (`modules/canopy/probe/diagnostic_portability/top.mbt:61-76`). | Failed full gate: both styles survive the semantic producer and renderer, but both producers/renderers are not established. This independently supports `NARROW`. |
...
| Public parser seam and standalone probe | `@lambda.parse_cst` returns `DiagnosticSet` (`loom/examples/lambda/pkg.generated.mbti:50`); the probe selects exact code, renders, applies, and reparses (`modules/canopy/probe/diagnostic_portability/top.mbt:4-76`). | Probe proves the narrow consumer path without CST/seam/editor imports. |
| Probe result | `moon test --release modules/canopy/probe/diagnostic_portability` passed: 1/1. The exact expected rendering, fixed text, and absent code are asserted at `modules/canopy/probe/diagnostic_portability/top.mbt:61-76`. | Concrete evidence for `if x then y`. |
...
Ran `moon check --deny-warn modules/canopy/probe/diagnostic_portability`,
`moon test --release modules/canopy/probe/diagnostic_portability`, scoped `moon fmt` and
