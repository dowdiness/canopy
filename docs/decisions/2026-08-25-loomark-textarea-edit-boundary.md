...
Editor mode changes keep the same textarea and native undo history. A multi-document Document switch instead replaces the textarea value once and starts a fresh native undo history without waiting for the previous document's save. Loomark does not promise cross-document undo: Chromium testing showed that same-page textareas share undo ordering, hidden textareas can block the focused textarea's undo, and programmatic value replacement cannot restore the previous document's history. Isolated browsing contexts preserve separate histories but would add focus, IME, accessibility, and lifecycle boundaries; editor-owned history would contradict this decision.
