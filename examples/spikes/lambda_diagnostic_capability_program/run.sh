...
cd "$ROOT"
SPIKE=examples/spikes/lambda_diagnostic_capability_program
REF=66a160f557f01f53e6e721016cf12d75b3d4fca4
INCR_SOURCE=${INCR_NEXT_SOURCE:-$ROOT/deps/loom/incr}
# The accepted annotation and decoration spikes are required bases, but
# descendants are allowed so these guards do not require an exact HEAD.
EXPECTED_BASE=09b3a6822c9db2d2a30b48c937a7a911906e73a0
EXPECTED_ANNOTATION_TREE=8baa417716281a0f32f18dd1a3c01a416870a39b
EXPECTED_DECORATION_TREE=8fc844236b95a7d9b6d2967774cf26ad246bb733
...
TEST_NAME=lambda_diagnostic_capability_program_wbtest
...
printf '%s\n' '== Lambda diagnostic capability Program evidence =='
git merge-base --is-ancestor "$EXPECTED_BASE" HEAD ||
  fail "Canopy HEAD does not contain accepted base $EXPECTED_BASE"
...
printf '%s\n' '-- exact production, prior spike, and submodule guards --'
...
[[ "$(git rev-parse HEAD:examples/spikes/lambda_annotation_capability_program)" == "$EXPECTED_ANNOTATION_TREE" ]] ||
  fail 'accepted #1219 annotation spike tree changed'
[[ "$(git rev-parse HEAD:examples/spikes/lambda_decoration_capability_program)" == "$EXPECTED_DECORATION_TREE" ]] ||
  fail 'accepted #1221 decoration spike tree changed'
if ! git diff --quiet -- examples/spikes/lambda_annotation_capability_program ||
  ! git diff --cached --quiet -- examples/spikes/lambda_annotation_capability_program ||
  git status --short -- examples/spikes/lambda_annotation_capability_program | grep -q .; then
  fail 'accepted #1219 annotation spike has working-tree changes'
fi
if ! git diff --quiet -- examples/spikes/lambda_decoration_capability_program ||
  ! git diff --cached --quiet -- examples/spikes/lambda_decoration_capability_program ||
  git status --short -- examples/spikes/lambda_decoration_capability_program | grep -q .; then
  fail 'accepted #1221 decoration spike has working-tree changes'
fi
...
printf '%s\n' '-- Formula callback purity source guard --'
FORMULA_REGION=$TEMP_ROOT/formula-callback.mbt
python3 - "$ROOT/$SPIKE/$TEST_NAME.mbt.disabled" "$FORMULA_REGION" <<'PY'
from pathlib import Path
import sys

source = Path(sys.argv[1]).read_text()
start_marker = "// FORMULA CALLBACK BEGIN"
end_marker = "// FORMULA CALLBACK END"
start = source.find(start_marker)
end = source.find(end_marker)
if start < 0 or end < 0 or end <= start:
    raise SystemExit("Formula callback markers are missing or reversed")
Path(sys.argv[2]).write_text(source[start:end + len(end_marker)])
PY
if rg -n '\bRef\b|analysis_projection|\.val\b|\bcompanion\b|\beditor\b|LambdaCompanion|AnalysisProjection' "$FORMULA_REGION"; then
  fail 'Formula callback reaches hidden host/editor state'
fi
python3 - "$FORMULA_REGION" <<'PY'
from pathlib import Path
import re
import sys

callback = Path(sys.argv[1]).read_text()
reads = [pos for pos in range(len(callback)) if callback.startswith("ctx.read(", pos)]
catch = callback.find("catch")
if len(reads) != 3:
    raise SystemExit(f"expected exactly three declared ctx.read calls, found {len(reads)}")
if catch < 0:
    raise SystemExit("Formula callback has no value-local catch")
if reads[-1] > catch:
    raise SystemExit("Formula callback catches a ctx.read or kernel operation")
if callback.count("catch") != 1:
    raise SystemExit("Formula callback has more than one catch boundary")
match = re.search(r"catch\s*\{(.*?)\n\s*\}", callback, re.S)
if not match:
    raise SystemExit("Formula callback catch block is not parseable")
catch_body = match.group(1)
constructors = (
    "SemanticDiagnosticError::InvalidSourceRange",
    "SemanticDiagnosticError::InvalidUtf16Boundary",
)
if catch_body.count("=>") != 2 or catch_body.count("=> None") != 2:
    raise SystemExit("Formula callback catch arm set is not exactly two None arms")
for constructor in constructors:
    if catch_body.count(constructor) != 1:
        raise SystemExit(f"Formula callback catch set is not exact for {constructor}")
value_start = callback.find("let value")
result_after = callback.find("Ok(value)", catch)
if value_start < 0 or value_start > catch or result_after < 0:
    raise SystemExit("catch is not lexically inside the computed value")
print("callback boundary: PASS (3 reads, semantic-only value catch)")
PY
...
# Materialize every exact support directory so the archived source symlinks
# resolve inside the disposable worktree.
...
if rg -n 'next_program|next_kernel|incremental_provider|FormulaBuilder|ReadPort|PatternState' \
...
if ! native_output=$(cd "$WORK" && NEW_MOON_MOD=0 moon test --target native "$COMPANION" --filter 'lambda diagnostic capability*' 2>&1); then
  printf '%s\n' "$native_output" >&2
  fail 'native candidate tests failed'
fi
printf '%s\n' "$native_output"
grep -Fq 'Total tests: 2, passed: 2, failed: 0.' <<<"$native_output" ||
  fail 'native candidate test count is not exactly 2/2'
if [[ "$wasm_supported" -eq 1 ]]; then
  if ! wasm_output=$(cd "$WORK" && NEW_MOON_MOD=0 moon test --target wasm-gc "$COMPANION" --filter 'lambda diagnostic capability*' 2>&1); then
    printf '%s\n' "$wasm_output" >&2
    fail 'wasm candidate tests failed'
  fi
  printf '%s\n' "$wasm_output"
  grep -Fq 'Total tests: 2, passed: 2, failed: 0.' <<<"$wasm_output" ||
    fail 'wasm candidate test count is not exactly 2/2'
  printf '%s\n' 'candidate exact output: lambda_diagnostic_capability_program native=PASS wasm-gc=PASS'
else
  printf '%s\n' 'candidate exact output: lambda_diagnostic_capability_program native=PASS wasm-gc=SKIP'
fi

printf '%s\n' '-- privacy, scope, and source guards --'
if rg -n '@incr|Decoration|pretty|Mount|Action|Resource|effects|resources' \
  "$ROOT/$SPIKE/$TEST_NAME.mbt.disabled"; then
  fail 'out-of-scope production/effect surface found in candidate source'
fi
if ! rg -q '@loom\.ParseSnapshot\[@ast\.Term\]' "$ROOT/$SPIKE/$TEST_NAME.mbt.disabled"; then
  fail 'candidate does not use the typed production parser snapshot'
fi
if ! rg -q 'SemanticDiagnosticError::Invalid(SourceRange|Utf16Boundary)' \
  "$ROOT/$SPIKE/$TEST_NAME.mbt.disabled"; then
  fail 'candidate does not narrow semantic validation failures'
fi
...
if ! git diff --quiet -- examples/spikes/lambda_annotation_capability_program ||
  ! git diff --cached --quiet -- examples/spikes/lambda_annotation_capability_program ||
  git status --short -- examples/spikes/lambda_annotation_capability_program | grep -q .; then
  fail 'accepted #1219 annotation spike changed during harness'
fi
changed_paths=$({
  git diff --name-only
...
forbidden_paths=$(grep -Ev '^examples/spikes/lambda_diagnostic_capability_program(/.*)?$' <<<"$changed_paths" || true)
if [[ -n "$forbidden_paths" ]]; then
  printf '%s\n' 'change outside the lambda diagnostic evidence allowlist:' >&2
  printf '%s\n' "$forbidden_paths" >&2
  exit 1
fi
...
printf '%s\n' "harness state: format=PASS source=#465/#469-exact submodules=PASS previous=#1219/#1221-PASS info=PASS privacy=PASS check-native=PASS check-wasm=$wasm_state test-native=PASS test-wasm=$wasm_state atomic=PASS provenance=PASS channels=PASS slopless=PASS production=PASS"
grep -Fq '**Pass with constraints.**' "$ROOT/$SPIKE/README.md"
printf '%s\n' 'constrained verdict: PASS - production raw diagnostic values and shadow Formula channels match for the finite workloads.'
