...
cd "$ROOT"
SPIKE=examples/spikes/lambda_decoration_capability_program
REF=66a160f557f01f53e6e721016cf12d75b3d4fca4
INCR_SOURCE=${INCR_NEXT_SOURCE:-$ROOT/deps/loom/incr}
# The accepted annotation spike is the required base, but descendants are
# allowed so this guard does not require an exact HEAD.
EXPECTED_BASE=0546799cff31cd8345bf01dbd7ec49954da6fc26
...
TEST_NAME=lambda_decoration_capability_program_wbtest
...
printf '%s\n' '== Lambda decoration capability Program evidence =='
...
native_output=$(cd "$WORK" && NEW_MOON_MOD=0 moon test --target native "$COMPANION" --filter 'lambda decoration capability*' 2>&1)
printf '%s\n' "$native_output"
...
if [[ "$wasm_supported" -eq 1 ]]; then
  wasm_output=$(cd "$WORK" && NEW_MOON_MOD=0 moon test --target wasm-gc "$COMPANION" --filter 'lambda decoration capability*' 2>&1)
  printf '%s\n' "$wasm_output"
  grep -Fq 'Total tests: 2, passed: 2, failed: 0.' <<<"$wasm_output" ||
    fail 'wasm candidate test count is not exactly 2/2'
  printf '%s\n' 'candidate exact output: lambda_decoration_capability_program native=PASS wasm-gc=PASS'
else
  printf '%s\n' 'candidate exact output: lambda_decoration_capability_program native=PASS wasm-gc=SKIP'
fi

printf '%s\n' '-- privacy, scope, and source guards --'
if rg -n '@incr|Diagnostic|pretty|Mount|Action|Resource|effects|resources' \
...
forbidden_paths=$(grep -Ev '^examples/spikes/lambda_decoration_capability_program(/.*)?$' <<<"$changed_paths" || true)
if [[ -n "$forbidden_paths" ]]; then
  printf '%s\n' 'change outside the lambda decoration evidence allowlist:' >&2
  printf '%s\n' "$forbidden_paths" >&2
  exit 1
fi
...
printf '%s\n' "harness state: format=PASS source=#465/#469-exact submodules=PASS previous=#1219-PASS info=PASS privacy=PASS check-native=PASS check-wasm=$wasm_state test-native=PASS test-wasm=$wasm_state atomic=PASS provenance=PASS slopless=PASS production=PASS"
grep -Fq '**Pass with constraints.**' "$ROOT/$SPIKE/README.md"
printf '%s\n' 'constrained verdict: PASS - production decoration closure and shadow Formula arrays match for the finite workloads.'
