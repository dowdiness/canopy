  ...
  for (const child of node.children) {
    if (nodeElementMap.has(child.node_id)) {
      ...
      if (childEl) {
        ...
        next ? container.insertBefore(childEl, next) : container.appendChild(childEl);
      }
    }
    idx++;
  }
...
function renderHtmlTree(data) {
  ...
  // First call: placeholder still present — clear and populate fresh
  if (htmlPreview.children.length === 0 || htmlPreview.querySelector('.no-parse')) {
    ...
    // Subsequent calls: ensure every child is in the DOM
    if (data.kind_tag === 'Root' || data.kind_tag === 'Fragment') {
      for (const c of data.children) {
        const e = nodeElementMap.get(c.node_id);
  ...
  try {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (step.success && step.root) {
        ...
        const hasTruncatedTag = step.errors?.some(e => e.startsWith('Truncated tag'));
